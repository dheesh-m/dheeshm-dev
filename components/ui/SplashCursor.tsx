...
export default function SplashCursor({
  ...
  // Keep dynamic color and fluid configs updated without recreating WebGL context
  useEffect(() => {
    configRef.current = {
      COLOR,
      RAINBOW_MODE,
  ...
  useEffect(() => {
    ...
    function getWebGLContext(targetCanvas: HTMLCanvasElement) {
      ...
      let ctx = targetCanvas.getContext("webgl2", params) as WebGL2RenderingContext | null;

      if (!ctx) {
        ctx = (targetCanvas.getContext("webgl", params) ||
      ...
      const isWebGL2 = "drawBuffers" in ctx;
    ...
    function initFramebuffers() {
      const simRes = getResolution(effectiveSimRes);
    ...
    function updateKeywords() {
      ...
      if (effectiveShading) displayKeywords.push("SHADING");
      displayMaterial.setKeywords(displayKeywords);
    }

    function getResolution(resolution: number) {
      const w = gl!.drawingBufferWidth || window.innerWidth;
      const h = gl!.drawingBufferHeight || window.innerHeight;
    ...
    function scaleByPixelRatio(input: number) {
      ...
      return Math.floor(input * Math.min(pixelRatio, 1.5));
    }
    ...
    function step(dt: number) {
      ...
      for (let i = 0; i < effectivePressureIters; i++) {
    ...
    function drawDisplay(target: FBO | null) {
      ...
      if (effectiveShading && displayMaterial.uniforms.texelSize) {
    ...
    function splat(x: number, y: number, dx: number, dy: number, color: ColorRGB) {
      ...
      if (splatProgram.uniforms.radius) {
        gl!.uniform1f(splatProgram.uniforms.radius, correctRadius(configRef.current.SPLAT_RADIUS / 100));
      }
      blit(velocity.write);
      velocity.swap();
  ...
  return (
    <div className={cn("fixed inset-0 pointer-events-none z-[1] w-full h-full overflow-hidden select-none", className)}>
      <canvas ref={canvasRef} id="fluid-canvas" className="w-full h-full block pointer-events-none" />
    </div>
  );
}
