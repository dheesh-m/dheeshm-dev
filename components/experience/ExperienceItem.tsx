    ...
    <div ref={containerRef} className="relative w-full">
      ...
      <div
        className={cn(
          ...
          "bg-[#FAFBFC]/95 backdrop-blur-2xl border-[#D9DEE4] hover:border-[#394E6E] shadow-[0_8px_30px_rgba(57,78,110,0.05)]",
          isOpen
            ? "border-[#394E6E] dark:border-[rgba(180,185,195,0.28)] shadow-[0_16px_44px_rgba(57,78,110,0.08)] dark:shadow-[0_16px_48px_rgba(0,0,0,0.65)]"
      ...
      >
        ...
        <div className="relative flex flex-col justify-between w-full p-5 sm:p-7 z-10">
          <div className="flex items-start justify-between gap-3 sm:gap-4 w-full">
            ...
            <div className="flex items-start gap-3 sm:gap-3.5 flex-1 min-w-0">
              {experience.logo && (
                <div className="relative w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-[#FAFBFC] dark:bg-[#1a1c23]/90 border border-[#D9DEE4] dark:border-white/15 p-1.5 shrink-0 flex items-center justify-center shadow-sm mt-0.5 group-hover:scale-105 transition-transform">
                  <Image
                    src={experience.logo}
              ...
              <div className="flex-1 min-w-0">
                <h3 className="text-base sm:text-lg md:text-xl font-bold text-[#171A1F] dark:text-[#f8fafc] font-display tracking-tight leading-snug">
                  {experience.role}{" "}
                  <span className="font-medium text-[#394E6E] dark:text-[#cbd5e1] transition-colors">
                    @ {experience.company}
                  </span>
                </h3>
                  ...
                  >
                    {experience.location && (
                      <div className="flex items-center gap-1.5 text-[#66717D] dark:text-gray-400">
                        <MapPin className="w-3.5 h-3.5 text-[#394E6E] dark:text-gray-400 shrink-0" />
                    ...
                    {experience.companyUrl && (
                      ...
                      >
                        <ExternalLink className="w-3.5 h-3.5 text-[#394E6E] dark:text-gray-400 shrink-0" />
            ...
            <div className="flex items-center gap-2.5 sm:gap-4 shrink-0 mt-0.5">
        ...
        <AnimatePresence initial={false}>
          {isOpen && (
            ...
            >
              <div className="px-5 sm:px-7 pb-6 sm:pb-8 pt-1 flex flex-col sm:flex-row items-center sm:items-start justify-between gap-5 sm:gap-6 md:gap-10 border-t border-[#D9DEE4] dark:border-white/[0.08]">
                ...
                <div className="flex-1 space-y-4 sm:space-y-5 pt-3 w-full">
                  ...
                  <div className="flex flex-wrap gap-1.5 sm:gap-2 pt-1">
                    {experience.technologies.map((tech) => (
                      <span
                        key={tech}
                        className="px-2.5 py-1 rounded-full text-[10px] sm:text-[11px] font-mono font-medium bg-[#E9EDF1] dark:bg-white/[0.05] border border-[#D9DEE4] dark:border-white/10 text-[#334155] dark:text-gray-300 hover:border-[#394E6E] dark:hover:border-white/25 transition-colors"
                      >
                        {tech}
                      </span>
                    ))}
                ...
                {/* Right Column (or centered on mobile): Company Logo Emblem with Concentric Orbital Rings */}
                {experience.logo && (
                  <div className="flex shrink-0 w-24 h-24 sm:w-28 sm:h-28 lg:w-32 lg:h-32 items-center justify-center relative my-2 sm:my-auto self-center sm:self-auto">
                    ...
                    <div className="absolute inset-2 sm:inset-3 rounded-full border border-[#D9DEE4] dark:border-white/10" />
                    
                    {/* Center Logo Disc */}
                    <div className="relative w-12 h-12 sm:w-14 sm:h-14 lg:w-16 lg:h-16 rounded-full bg-[#FAFBFC] dark:bg-[#1a1c23]/90 backdrop-blur-md border border-[#D9DEE4] dark:border-white/15 p-2 flex items-center justify-center shadow-md group-hover:scale-105 transition-transform">
                      <Image
                        src={experience.logo}
                        alt={`${experience.company} logo`}
                        width={64}
